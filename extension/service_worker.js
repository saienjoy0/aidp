...
const FEATURE_FLAGS = Object.freeze({
  update_region: true,
  set_labels: false,
  // Beta 18: current AIDP template contract is verified from live raw_uidl.
  // Normal add/split lets AIDP's template onChange generate the production ID;
  // delete mutates the controlled form.regions reference and commits through the
  // same native onChange. Every structural operation still requires dry-run and
  // individual user approval, and label/speaker automation remains disabled.
  split_region: true,
  add_region: true,
  delete_region: true
});
...
// Beta 18 native structural adapter.
// Normal add never invents a production region ID: it calls the template's
// controlled onChange with a transient non-production ID and then resolves the
// formal `region_<Date.now>_<random>` ID generated by AIDP itself.
// Delete and recovery exact-add operate on the controlled `form.regions`
// reference exposed as the wavesurfer `regions` prop, then call the same AIDP
// template onChange so sorting/round_id/checkOverlap/autosave follow native code.
async function performNativeControlledStructureMainWorld(payload) {
  const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
  const round = value => Number.isFinite(Number(value)) ? Number(Number(value).toFixed(9)) : null;
  const close = (a, b, eps = 0.0025) => Number.isFinite(Number(a)) && Number.isFinite(Number(b)) && Math.abs(Number(a) - Number(b)) <= eps;
  const FORMAL_ID_RE = /^region_\d+_[a-z0-9]+$/i;

  const plain = region => {
    if (!region) return null;
    const out = {};
    const pairs = [
      ['id', 'id'], ['start', 'start'], ['end', 'end'], ['yuan_text', 'yuan_text'],
      ['if_save', 'if_save'], ['is_qualified', 'is_qualified'], ['music', 'music'],
      ['speaker_desc', 'speaker_desc'], ['round_id', 'round_id'], ['color', 'color'],
      ['drag', 'drag'], ['resize', 'resize'], ['loop', 'loop']
    ];
    for (const [source, dest] of pairs) {
      try {
        if (region?.[source] !== undefined) out[dest] = ['start', 'end'].includes(source) ? round(region[source]) : region[source];
      } catch (_) {}
    }
    return out;
  };
    ...
    for (const seed of seeds) {
      ...
      for (let depth = 0; fiber && depth < 90; depth += 1, fiber = fiber.return) {
        ...
        for (const props of [fiber.memoizedProps, fiber.pendingProps]) {
          if (!props || typeof props !== 'object') continue;
          for (const variant of [props, props.$api?.props].filter(Boolean)) {
            const regionsRef = variant.regions;
            let regions = [];
            try {
              if (regionsRef && typeof regionsRef[Symbol.iterator] === 'function') regions = Array.from(regionsRef);
            } catch (_) {}
            const api = props.$api || variant.$api || null;
            const ref = api?.componentRef?.current || api?.$componentRef?.current || null;
            let score = 0;
            if (regions.length) score += 8;
            if (variant.regionsControlled === true) score += 12;
            if (typeof variant.onChange === 'function') score += 16;
            if (typeof ref?.getWavesurferInstance === 'function') score += 5;
            if (/wavesurfer/i.test(String(api?.id || api?.$xpath || fiber.type?.displayName || fiber.elementType?.displayName || ''))) score += 8;
            if (score >= 28) candidates.push({ score, regionsRef, regions, ref, api, props: variant, fiber });
          }
        }
      }
    }
  ...
  const readTemplateContract = startFiber => {
    let fiber = startFiber;
    for (let depth = 0; fiber && depth < 180; depth += 1, fiber = fiber.return) {
      for (const props of [fiber.memoizedProps, fiber.pendingProps]) {
        const raw = props?.neekoTemplateSchema?.raw_uidl;
        if (typeof raw !== 'string' || !raw.includes('neeko-wavesurfer')) continue;
        return {
          found: true,
          native_add_id: raw.includes('region_${Date.now()}_${Math.random().toString(36).substring(2, 9)}'),
          native_defaults: raw.includes('speaker_desc: \\"1\\"') || raw.includes('speaker_desc: "1"'),
          renumber: raw.includes('item.round_id = index + 1') && raw.includes('newFormRegions.sort'),
          filter_lock: raw.includes('ctx.innerState.filterSpeaker'),
          controlled_regions: raw.includes('form.regions'),
          delete_contract: raw.includes('ctx.form.regions = next') && raw.includes('list.filter(r => r.id !== sel)')
        };
      }
    }
    return { found: false };
  ...
  const readFilterSpeaker = () => {
    const seeds = [...document.querySelectorAll('[data-component-rum-id="neeko-multi-select"],.arco-select,.arco-select-view')];
    const seen = new Set();
    for (const seed of seeds) {
      let fiber = fiberOf(seed);
      for (let depth = 0; fiber && depth < 35; depth += 1, fiber = fiber.return) {
        ...
        for (const props of [fiber.memoizedProps, fiber.pendingProps]) {
          ...
          for (const variant of [props, props.$api?.props].filter(Boolean)) {
            const api = props.$api || variant.$api || null;
            const identity = String(api?.id || api?.$xpath || variant?.$componentCustomId || '');
            if (!identity.includes('neeko-multi-select_8764j4')) continue;
            let value = variant.value;
            try { if (value && typeof value[Symbol.iterator] === 'function' && typeof value !== 'string') value = Array.from(value); } catch (_) {}
            if (Array.isArray(value)) return { found: true, value: value.map(v => String(v)) };
            if (value == null || value === '') return { found: true, value: [] };
  ...
  const target = findTarget();
  if (!target) return { ok: false, error: 'AIDP controlled wavesurferを検出できません' };
  if (target.props?.regionsControlled !== true) return { ok: false, error: 'AIDP wavesurferがcontrolled modeではありません' };
  if (typeof target.props?.onChange !== 'function') return { ok: false, error: 'AIDP template onChangeを取得できません' };

  const contract = readTemplateContract(target.fiber);
  if (!contract.found || !contract.controlled_regions || !contract.renumber || !contract.filter_lock) {
    return { ok: false, error: '現在テンプレートのstructural contractがBeta18既知仕様と一致しません', contract };
  }
  const filter = readFilterSpeaker();
  if (!filter.found) return { ok: false, error: '話者フィルター状態を安全に確認できません。構造操作を停止します', contract };
  if (filter.value.length) return { ok: false, error: '話者フィルター中はAIDP自身が切分・時間変更をロックします。フィルターを解除してください', filter_speaker: filter.value, contract };

  const action = String(payload?.action || '');
  const beforeRegions = target.regions.map(plain).filter(Boolean);
  const beforeIds = beforeRegions.map(r => String(r.id || ''));
  const beforeIdSet = new Set(beforeIds);

  if (action === 'precheck') {
    return { ok: true, adapter: 'aidp-native-template-structural-precheck-v1', action, filter_speaker: filter.value, contract };
  }

  if (action === 'native_add') {
    if (!contract.native_add_id || !contract.native_defaults) {
      return { ok: false, error: 'AIDP native region ID/default生成式を現在テンプレートで確認できません', contract };
    }
    const start = Number(payload?.region?.start);
    const end = Number(payload?.region?.end);
    if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start) return { ok: false, error: 'native_addのstart/endが不正です' };
    const transientId = `__aidp_bridge_native_runtime__${String(payload?.op_id || 'op').replace(/[^A-Za-z0-9_-]/g, '_').slice(0, 60)}__${Date.now()}__${Math.random().toString(36).slice(2, 8)}`;
    const eventRegions = target.regions.map(region => ({ id: String(region?.id || ''), start: Number(region?.start), end: Number(region?.end) }));
    eventRegions.push({ id: transientId, start, end });
    try {
      await target.props.onChange(eventRegions);
    } catch (error) {
      return { ok: false, error: error?.message || String(error), action, contract, transient_id: transientId };
    }
    ...
    for (let attempt = 0; attempt < 60; attempt += 1) {
      await sleep(150);
      refreshed = findTarget() || refreshed;
      const after = refreshed.regions.map(plain).filter(Boolean);
      newIds = after.map(r => String(r.id || '')).filter(id => id && !beforeIdSet.has(id));
      const candidates = after.filter(r => newIds.includes(String(r.id || '')) && FORMAL_ID_RE.test(String(r.id || '')) && close(r.start, start) && close(r.end, end));
      if (candidates.length === 1 && newIds.length === 1) {
        added = candidates[0];
        break;
      }
    }
    if (!added) {
      return {
        ok: false,
        error: `AIDP native add後の正式IDを一意に確定できません（newIds=${newIds.join(',') || 'none'}）`,
        action,
        contract,
    ...
    if (String(added.id) === transientId || !FORMAL_ID_RE.test(String(added.id))) {
      return { ok: false, error: '一時IDが正式IDとして残りました。適用を停止します', action, added, transient_id: transientId };
    }
    const defaultsOk = String(added.speaker_desc ?? '') === '1' && String(added.if_save ?? '') === '保留' && String(added.is_qualified ?? '') === '无问题' && String(added.music ?? '') === '说话';
    if (!defaultsOk) {
      return { ok: false, error: 'AIDP native addの初期metadataが既知仕様と一致しません', action, added, contract };
    }
    let wavePresent = null;
    try {
      const ws = refreshed.ref?.getWavesurferInstance?.();
      wavePresent = Boolean(ws?.regions?.list?.[added.id]);
    } catch (_) {}
    return {
      ok: true,
      adapter: 'aidp-native-template-onChange-add-v1',
      action,
      formal_id_generated_by_aidp: true,
      resolved_region_id: String(added.id),
      transient_id: transientId,
      transient_id_persisted: false,
      before_ids: beforeIds,
      after_ids: refreshed.regions.map(r => String(r?.id || '')),
      after: added,
      wave_present: wavePresent,
      contract
    };
  ...
  if (action === 'delete' || action === 'exact_add') {
    if (!contract.delete_contract && action === 'delete') {
      return { ok: false, error: 'AIDP delete contractを現在テンプレートで確認できません', contract };
    }
    const regionsRef = target.regionsRef;
    if (!regionsRef || typeof regionsRef.splice !== 'function' || typeof regionsRef.sort !== 'function') {
      return { ok: false, error: 'controlled form.regions参照を可変配列として取得できません' };
    }
    const normalizeOrder = list => {
      list.sort((a, b) => Number(a?.start) - Number(b?.start));
      Array.from(list).forEach((item, index) => {
        try { item.round_id = index + 1; } catch (_) {}
      });
    };

    if (action === 'delete') {
      const regionId = String(payload?.region_id || payload?.region?.region_id || payload?.region?.id || '');
      const index = Array.from(regionsRef).findIndex(region => String(region?.id || '') === regionId);
      if (!regionId || index < 0) return { ok: false, error: `削除対象regionが見つかりません: ${regionId}` };
      const removedRaw = regionsRef[index];
      const removed = plain(removedRaw);
      try {
        regionsRef.splice(index, 1);
        normalizeOrder(regionsRef);
        await target.props.onChange(Array.from(regionsRef));
      } catch (error) {
        try {
          if (!Array.from(regionsRef).some(r => String(r?.id || '') === regionId)) {
            regionsRef.push(removedRaw);
            normalizeOrder(regionsRef);
            await target.props.onChange(Array.from(regionsRef));
          }
        } catch (_) {}
        return { ok: false, error: error?.message || String(error), action, region_id: regionId, before: removed, contract };
      }
      ...
      for (let attempt = 0; attempt < 60; attempt += 1) {
        await sleep(150);
        refreshed = findTarget() || refreshed;
        if (!refreshed.regions.some(region => String(region?.id || '') === regionId)) break;
      }
      const stillPresent = refreshed.regions.some(region => String(region?.id || '') === regionId);
      if (stillPresent) {
        return { ok: false, error: `controlled deleteがModelへ反映されませんでした: ${regionId}`, action, region_id: regionId, before: removed, contract };
      }
      let wavePresent = null;
      try {
        const ws = refreshed.ref?.getWavesurferInstance?.();
      ...
      return {
        ok: true,
        adapter: 'aidp-controlled-form-regions-delete-v1',
        action,
        region_id: regionId,
        before: removed,
        before_ids: beforeIds,
    ...
    const input = payload?.region || {};
    const regionId = String(input.region_id || input.id || '');
    if (!regionId) return { ok: false, error: 'exact_addのregion IDがありません' };
    if (beforeIdSet.has(regionId)) return { ok: true, adapter: 'aidp-controlled-form-regions-exact-add-v1', action, region_id: regionId, already_present: true, contract };
    const requested = {
      id: regionId,
      start: Number(input.start),
      end: Number(input.end),
      yuan_text: String(input.text ?? input.yuan_text ?? ''),
      if_save: String(input.keep ?? input.if_save ?? '保留'),
      is_qualified: String(input.quality ?? input.is_qualified ?? '无问题'),
      music: String(input.voice_type ?? input.music ?? '说话'),
      speaker_desc: String(input.speaker ?? input.speaker_desc ?? '1'),
      round_id: Number.isFinite(Number(input.round_id)) ? Number(input.round_id) : null
    };
    if (!Number.isFinite(requested.start) || !Number.isFinite(requested.end) || requested.end <= requested.start) {
      return { ok: false, error: 'exact_addのstart/endが不正です' };
    }
    try {
      regionsRef.push(requested);
      normalizeOrder(regionsRef);
      await target.props.onChange(Array.from(regionsRef));
    } catch (error) {
      try {
        const idx = Array.from(regionsRef).findIndex(r => String(r?.id || '') === regionId);
        if (idx >= 0) regionsRef.splice(idx, 1);
        normalizeOrder(regionsRef);
        await target.props.onChange(Array.from(regionsRef));
      } catch (_) {}
      return { ok: false, error: error?.message || String(error), action, region_id: regionId, requested: plain(requested), contract };
    }
    let refreshed = target;
    let exact = null;
    for (let attempt = 0; attempt < 60; attempt += 1) {
      await sleep(150);
      refreshed = findTarget() || refreshed;
      exact = refreshed.regions.find(region => String(region?.id || '') === regionId) || null;
      if (exact) break;
    }
    if (!exact) {
      const afterIds = refreshed.regions.map(r => String(r?.id || ''));
      const unexpected = afterIds.filter(id => !beforeIdSet.has(id));
      return { ok: false, error: `exact_addで元IDを復元できませんでした: ${regionId}`, action, region_id: regionId, unexpected_new_ids: unexpected, contract };
    }
    return {
      ok: true,
      adapter: 'aidp-controlled-form-regions-exact-add-v1',
      action,
      region_id: regionId,
      requested: plain(requested),
      after: plain(exact),
  ...
  return { ok: false, error: `未対応native structural action: ${action}` };
}

async function performNativeControlledStructure(tabId, action, payload = {}) {
  const result = await chrome.scripting.executeScript({
    target: { tabId },
    world: 'MAIN',
    func: performNativeControlledStructureMainWorld,
    args: [{ action, ...payload }]
  });
  const value = result?.[0]?.result;
  if (!value?.ok) {
    const error = new Error(value?.error || `AIDP native structural操作に失敗しました: ${action}`);
    error.code = 'AIDP_NATIVE_STRUCTURAL_FAILED';
    error.adapter_result = value || null;
    throw error;
  }
  return value;
}
...
async function performRegionUpdate(tabId, operation) {
  ...
  if (hasNeekoUpdate) {
    // AIDP's live template explicitly locks timestamp/structural edits while
...
function replaceStructuralRegionIdDeep(value, fromId, toId, seen = new WeakSet()) {
  if (!value || typeof value !== 'object') return value;
  if (seen.has(value)) return value;
  seen.add(value);
  if (Array.isArray(value)) {
    for (const item of value) replaceStructuralRegionIdDeep(item, fromId, toId, seen);
    return value;
  }
  for (const key of Object.keys(value)) {
    const current = value[key];
    if ((key === 'region_id' || key === 'id') && String(current || '') === fromId) {
      value[key] = toId;
    } else if (current && typeof current === 'object') {
      replaceStructuralRegionIdDeep(current, fromId, toId, seen);
    }
  }
  return value;
}

async function resolveStructuralPlaceholderInJournal(journal, placeholderId, actualId) {
  const fromId = String(placeholderId || '');
  const toId = String(actualId || '');
  if (!isStructuralPlaceholderId(fromId)) throw new Error(`構造placeholderではありません: ${fromId}`);
  if (!/^region_\d+_[a-z0-9]+$/i.test(toId)) throw new Error(`AIDP正式region ID形式ではありません: ${toId}`);
  journal.resolved_structural_ids = journal.resolved_structural_ids || {};
  const already = journal.resolved_structural_ids[fromId];
  if (already && already !== toId) throw new Error(`placeholderの解決先が競合しています: ${fromId} -> ${already} / ${toId}`);
  journal.resolved_structural_ids[fromId] = toId;
  replaceStructuralRegionIdDeep(journal.operations, fromId, toId);
  await writeJournal(journal);
  await refreshJournalExpectedFingerprint(journal);
  return toId;
}

function finalExpectedRegionsFromJournal(journal) {
  const operations = Array.isArray(journal?.operations) ? journal.operations : [];
  for (let index = operations.length - 1; index >= 0; index -= 1) {
    const expected = operations[index]?.expected_regions_after;
    if (Array.isArray(expected)) return expected;
  }
  return Array.isArray(journal?.backup_regions) ? journal.backup_regions : [];
}

async function refreshJournalExpectedFingerprint(journal) {
  const expected = finalExpectedRegionsFromJournal(journal);
  const unresolved = expected.filter(region => isStructuralPlaceholderId(region?.region_id)).map(region => region.region_id);
  if (unresolved.length) {
    journal.expected_result_fingerprint = null;
    journal.unresolved_structural_ids = [...new Set(unresolved)];
  } else {
    journal.expected_result_fingerprint = `sha256:${await sha256Text(JSON.stringify(expected))}`;
    journal.unresolved_structural_ids = [];
  }
  await writeJournal(journal);
  return journal.expected_result_fingerprint;
}
...
async function performStructuralAdd(tabId, operation, context = {}) {
  ...
  if (!journal) throw new Error('add_regionのjournal contextがありません');
  const placeholderId = String(region.region_id);
  const requestedBeforeResolution = cloneJson(region);
  const traceId = `save-struct-add-${Date.now()}-${Math.random().toString(16).slice(2, 10)}`;
  const result = {
    ok: true,
    adapter: 'aidp-native-structural-add-v1',
    placeholder_region_id: placeholderId,
    region_id: null,
    native: null,
    text: null,
    save: null,
  ...
  try {
    ...
    actualId = String(result.native?.resolved_region_id || '');
    if (!actualId) throw new Error('AIDP native addの正式region IDを取得できません');

    // Crash recovery must know the real ID before any later text/update step.
    await resolveStructuralPlaceholderInJournal(journal, placeholderId, actualId);
    operation = journal.operations.find(item => item.op_id === operation.op_id) || operation;
    result.region_id = actualId;

    const resolvedRegion = operation.after;
    result.text = await performTextUpdate(tabId, {
      region_id: actualId,
      set: { text: String(resolvedRegion?.text ?? requestedBeforeResolution.text ?? '') },
      before: { ...cloneJson(resolvedRegion || requestedBeforeResolution), text: '' },
      after: resolvedRegion
    });
    ...
    if (actualId) {
      try {
        await performNativeControlledStructure(tabId, 'delete', {
          region_id: actualId,
          region: { ...requestedBeforeResolution, region_id: actualId }
        });
      } catch (compensationError) {
        compensationErrors.push(`追加補償削除失敗: ${compensationError?.message || String(compensationError)}`);
      }
    }
    result.compensated = Boolean(actualId) && compensationErrors.length === 0;
    const wrapped = new Error([error?.message || String(error), ...compensationErrors].join(' / '));
    wrapped.code = error?.code;
    wrapped.trace = error?.trace;
    wrapped.adapter_result = result;
...
async function performStructuralDelete(tabId, operation) {
  ...
  const result = {
    ok: true,
    adapter: 'aidp-native-controlled-delete-v1',
    region_id: region.region_id,
    native: null,
    save: null,
    compensated: false
  ...
  try {
    ...
    const compensationErrors = [];
    if (removed) {
      try { await performNativeControlledStructure(tabId, 'exact_add', { region }); }
...
async function performStructuralSplit(tabId, operation, context = {}) {
  ...
  if (!second?.region_id || !isStructuralPlaceholderId(second.region_id)) {
    throw new Error('split_regionの第2partにnative placeholder IDがありません');
  }
  const placeholderId = String(second.region_id);
  const requestedSecondBeforeResolution = cloneJson(second);
  const result = {
    ok: true,
    adapter: 'aidp-native-structural-split-v1',
    region_id: operation.region_id,
    placeholder_region_id: placeholderId,
    added_region_id: null,
    add: null,
    add_text: null,
    update_first: null,
    compensated: false
  ...
  try {
    ...
    try {
      result.add = await performNativeControlledStructure(tabId, 'native_add', {
        op_id: operation.op_id,
        region: { start: second.start, end: second.end }
      });
      actualSecondId = String(result.add?.resolved_region_id || '');
      if (!actualSecondId) throw new Error('split_regionのnative第2part正式IDを取得できません');

      await resolveStructuralPlaceholderInJournal(journal, placeholderId, actualSecondId);
      operation = journal.operations.find(item => item.op_id === operation.op_id) || operation;
      [first, second] = operation.after;
      result.added_region_id = actualSecondId;

      result.add_text = await performTextUpdate(tabId, {
        region_id: actualSecondId,
        set: { text: String(second.text ?? requestedSecondBeforeResolution.text ?? '') },
        before: { ...cloneJson(second), text: '' },
        after: second
      });

      // The first save can legitimately still contain the unsplit first-part
      // timing. We only require the new native region and its text here; the
      // full-state save is verified by performRegionUpdate below.
    ...
    if (actualSecondId) {
      try {
        await performNativeControlledStructure(tabId, 'delete', {
          region_id: actualSecondId,
          region: { ...requestedSecondBeforeResolution, region_id: actualSecondId }
        });
      } catch (compensationError) {
        compensationErrors.push(`分割追加region削除失敗: ${compensationError?.message || String(compensationError)}`);
      }
    }
    result.compensated = (firstUpdated || Boolean(actualSecondId)) && compensationErrors.length === 0;
    const wrapped = new Error([error?.message || String(error), ...compensationErrors].join(' / '));
    wrapped.code = error?.code;
    wrapped.trace = error?.trace;
    wrapped.adapter_result = result;
...
async function performPatchOperation(tabId, operation, context = {}) {
  ...
  if (operation.type === 'split_region') return performStructuralSplit(tabId, operation, context);
...
async function performRecoveryStructuralMutation(tabId, action, region, expectedCount) {
  ...
  try {
    if (action === 'remove') {
      result.neeko = await performNativeControlledStructure(tabId, 'delete', { region_id: regionId, region });
    } else if (action === 'add') {
      result.neeko = await performNativeControlledStructure(tabId, 'exact_add', { region });
    } else {
      throw new Error(`recovery structural action未対応: ${action}`);
    }
...
async function runPatchDryRun(patchInput, approvedStructuralOpIds = null) {
  ...
  const needsNativePrecheck = (report.operations || []).some(item => {
    if (['split_region', 'add_region', 'delete_region'].includes(item.type) && !(item.errors || []).length) return true;
    if (item.type === 'update_region') return (item.changes || []).some(change => ['start', 'end'].includes(change.field));
    return false;
  });
  if (needsNativePrecheck) {
    try {
      report.native_structural_precheck = await performNativeControlledStructure(snapshot.tab.id, 'precheck');
    } catch (error) {
      report.native_structural_precheck = { ok: false, error: error?.message || String(error) };
      report.errors.unshift(`AIDP native構造precheck失敗: ${error?.message || String(error)}`);
      report.applicable = false;
    }
  }
...
async function applyPatchFromDryRun(token) {
  ...
  const journal = {
    ...
    dry_run_expected_result_fingerprint_provisional: dryRun.expected_result_fingerprint,
    expected_result_fingerprint: null,
    resolved_structural_ids: {},
  ...
  try {
    ...
    for (let index = 0; index < journal.operations.length; index += 1) {
      ...
      try {
        operationResult = await performPatchOperation(tab.id, item, { journal });
    ...
    if (current.caseData.source_fingerprint !== journal.expected_result_fingerprint) {
      throw new Error('全操作後fingerprintがAIDP正式ID解決後の予定値と一致しません');
    }
